...
| **Cumplen Convención (`✅ Mantener`)** | 100 (100%) | 79 (100%) | **179** (100%) |
| **Requieren Renombre (`🔄 Renombrar`)** | 0 (42 ✅ aplicados) | 0 (17 ✅ aplicados) | **0** (59 ✅ aplicados) |
| **Requieren Reubicación (`📦 Reubicar`)** | 0 | 0 (1 ✅ aplicada) | **0** (1 ✅ aplicada) |
| **Estado General** | ✅ Backend 100% Completado | ✅ Frontend 100% Completado | 🏆 **100% Completado (179/179 Conformes)** |
...
| 2 | `locale_defaults.py` | `locale_defaults.py` | ✅ Mantener | ✅ Aplicado | Evita adjetivo antes de sustantivo; centraliza valores por defecto. |
...
| 2 | `alerts_storage.py` | `alerts_storage.py` | ✅ Mantener | ✅ Aplicado | Plural para mantener paridad con `alerts_controller` y `alerts_view`. |
...
| 11 | `logs_storage.py` | `logs_storage.py` | ✅ Mantener | ✅ Aplicado | Estandariza con `logs_controller` y `logs_view`. |
| 12 | `timers_storage.py` | `timers_storage.py` | ✅ Mantener | ⚠️ | Persistencia de temporizadores automáticos de chat. |
| 13 | `tokens_storage.py` | `tokens_storage.py` | ✅ Mantener | ✅ Aplicado | Plural para coincidir con la naturaleza de colección de credenciales. |
...
| # | Archivo Actual | Nombre Estandarizado | Acción | Estado | Justificación / Notas |
|---|---|---|---|---|---|
| 1 | `spam_handler.py` | `spam_handler.py` | ✅ Mantener | ✅ Aplicado | El dominio es spam/moderación; elimina "filter" redundante. |
| 2 | `logs_handler.py` | `logs_handler.py` | ✅ Mantener | ✅ Aplicado | Plural consistente con `logs_controller` y `logs_storage`. |
| 3 | `music_handler.py` | `music_handler.py` | ✅ Mantener | ✅ Aplicado | El dominio es music; elimina "command" redundante. |
| 4 | `tts_handler.py` | `tts_handler.py` | ✅ Mantener | ✅ Aplicado | El dominio es tts; elimina "voice" redundante. |
...
| # | Archivo Actual | Nombre Estandarizado | Acción | Estado | Justificación / Notas |
|---|---|---|---|---|---|
| 1 | `alerts_models.py` | `alerts_models.py` | ✅ Mantener | ✅ Aplicado | Plural consistente con `alerts_storage` y `alerts_controller`. |
...
| # | Archivo Actual | Nombre Estandarizado | Acción | Estado | Justificación / Notas |
|---|---|---|---|---|---|
| 1 | `kick_provider.py` | `kick_provider.py` | ✅ Mantener | ✅ Aplicado | Estandariza con el sufijo `_provider` para la API de Kick. |
| 2 | `kick_ws_provider.py` | `kick_ws_provider.py` | ✅ Mantener | ✅ Aplicado | Diferencia explícitamente el proveedor de socket en tiempo real. |
| 3 | `tiktok_provider.py` | `tiktok_provider.py` | ✅ Mantener | ✅ Aplicado | Elimina "chat" redundante dado que está en la carpeta `chat/`. |
| 4 | `twitch_provider.py` | `twitch_provider.py` | ✅ Mantener | ✅ Aplicado | Estandariza con el sufijo `_provider` para la API de Twitch. |
| 5 | `twitch_ws_provider.py` | `twitch_ws_provider.py` | ✅ Mantener | ✅ Aplicado | Diferencia explícitamente el socket de Twitch. |
| 6 | `youtube_provider.py` | `youtube_provider.py` | ✅ Mantener | ✅ Aplicado | Elimina "chat" redundante en el nombre. |

#### `providers/music/` (1 archivo)
| # | Archivo Actual | Nombre Estandarizado | Acción | Estado | Justificación / Notas |
|---|---|---|---|---|---|
| 7 | `youtube_provider.py` | `youtube_provider.py` | ✅ Mantener | ✅ Aplicado | Estandariza como proveedor de música según `i_music_provider.py`. |

#### `providers/voices/` (3 archivos)
| # | Archivo Actual | Nombre Estandarizado | Acción | Estado | Justificación / Notas |
|---|---|---|---|---|---|
| 8 | `local_provider.py` | `local_provider.py` | ✅ Mantener | ✅ Aplicado | Elimina prefijo redundante `tts_`; adopta sufijo `_provider`. |
| 9 | `online_provider.py` | `online_provider.py` | ✅ Mantener | ✅ Aplicado | Elimina prefijo `tts_`; adopta sufijo `_provider`. |
| 10 | `piper_provider.py` | `piper_provider.py` | ✅ Mantener | ✅ Aplicado | Elimina prefijo `tts_`; adopta sufijo `_provider`. |
...
| # | Archivo Actual | Nombre Estandarizado | Acción | Estado | Justificación / Notas |
|---|---|---|---|---|---|
| 1 | `alerts_queue.py` | `alerts_queue.py` | ✅ Mantener | ✅ Aplicado | Plural coherente con el subsistema `alerts`. |
| 2 | `alerts_service.py` | `alerts_service.py` | ✅ Mantener | ✅ Aplicado | Plural coherente con `alerts_controller` y `alerts_storage`. |

#### `services/auth/` (1 archivo)
| # | Archivo Actual | Nombre Estandarizado | Acción | Estado | Justificación / Notas |
|---|---|---|---|---|---|
| 3 | `auth_service.py` | `auth_service.py` | ✅ Mantener | ✅ Aplicado | Coincide exactamente con la carpeta `auth/` y la interfaz `i_auth.py`. |

#### `services/chat/` (7 archivos)
| # | Archivo Actual | Nombre Estandarizado | Acción | Estado | Justificación / Notas |
|---|---|---|---|---|---|
| 4 | `chat_service.py` | `chat_service.py` | ✅ Mantener | ⚠️ | Servicio de mensajería en singular. |
| 5 | `commands_service.py` | `commands_service.py` | ✅ Mantener | ✅ Aplicado | Plural coherente con `commands_controller` y `commands_storage`. |
| 6 | `chat_pipeline.py` | `chat_pipeline.py` | ✅ Mantener | ✅ Aplicado | Añade dominio al inicio para evitar ambigüedad. |
| 7 | `piper_manager.py` | `piper_manager.py` | ✅ Mantener | ✅ Aplicado | Elimina "voice" redundante en el contexto del motor Piper. |
| 8 | `spam_service.py` | `spam_service.py` | ✅ Mantener | ⚠️ | Servicio de moderación en singular. |
| 9 | `timers_service.py` | `timers_service.py` | ✅ Mantener | ✅ Aplicado | Plural coherente con `timers_controller` y `timers_storage`. |
...
| 13 | `overlay_ws_client.py` | `overlay_ws_client.py` | ✅ Mantener | ✅ Aplicado | Agrega prefijo de dominio `overlay_` para no confundir con chat. |
...
| 20 | `instance_service.py` | `instance_service.py` | ✅ Mantener | ✅ Aplicado | Singular consistente con todos los demás servicios (`_service.py`). |
| 21 | `logs_service.py` | `logs_service.py` | ✅ Mantener | ✅ Aplicado | Plural coherente con `logs_controller` y `logs_storage`. |
...
| 25 | `widgets_service.py` | `widgets_service.py` | ✅ Mantener | ✅ Aplicado | Plural coherente con `widgets_controller` y `widgets_storage`. |
...
| 13 | `twitch_rewards_worker.py` | `twitch_rewards_worker.py` | ✅ Mantener | ✅ Aplicado | Plural coherente con `rewards_worker.py`. |
| 14 | `updater_worker.py` | `updater_worker.py` | ✅ Mantener | ✅ Aplicado | Coherente con `updater_controller.py` y `updater_service.py`. |
...
| # | Archivo Actual | Nombre Estandarizado | Acción | Estado | Justificación / Notas |
|---|---|---|---|---|---|
| 18 | `logs_controls.py` | `logs_controls.py` | ✅ Mantener | ✅ Aplicado | Plural coherente con `logs_view.py`. |
...
| # | Archivo Actual | Nombre Estandarizado | Acción | Estado | Justificación / Notas |
|---|---|---|---|---|---|
| 28 | `widget_card.py` | `widget_card.py` | ✅ Mantener | ✅ Aplicado | Elimina el sufijo redundante `_component`. |
...
| 4 | `commands_dialog.py` | `commands_dialog.py` | ✅ Mantener | ✅ Aplicado | Plural coherente con `commands_controller` y `commands_view`. |
| 5 | `crash_report_dialog.py` | `crash_report_dialog.py` | ✅ Mantener | ⚠️ | Diálogo de informe de cierre inesperado. |
| 6 | `message_dialog.py` | `message_dialog.py` | ✅ Mantener | ✅ Aplicado | Elimina "editor" redundante. |
| 7 | `piper_dialog.py` | `piper_dialog.py` | ✅ Mantener | ✅ Aplicado | Elimina "voices" redundante en contexto de TTS. |
| 8 | `platform_dialog.py` | `platform_dialog.py` | ✅ Mantener | ✅ Aplicado | Elimina "connect" redundante. |
...
| 11 | `tiktok_dialog.py` | `tiktok_dialog.py` | ✅ Mantener | ✅ Aplicado | Elimina "connect" redundante. |
| 12 | `timers_dialog.py` | `timers_dialog.py` | ✅ Mantener | ✅ Aplicado | Plural coherente con `timers_controller` y `timers_view`. |
| 13 | `updater_dialog.py` | `updater_dialog.py` | ✅ Mantener | ✅ Aplicado | Coherente con `updater_controller` y `updater_service`. |
| 14 | `positioner_dialog.py` | `positioner_dialog.py` | ✅ Mantener | ✅ Aplicado | Elimina "visual" redundante. |
| 15 | `youtube_dialog.py` | `youtube_dialog.py` | ✅ Mantener | ✅ Aplicado | Elimina "connect" redundante. |
...
### 5. `frontend/views/` (13 archivos)

| # | Archivo Actual | Nombre Estandarizado | Acción | Estado | Justificación / Notas |
|---|---|---|---|---|---|
| 1 | `base_view.py` | `base_view.py` | ✅ Mantener | ✅ Aplicado | Reubicado desde `widgets/`. Clase base de vistas. |
| 2 | `alerts_view.py` | `alerts_view.py` | ✅ Mantener | ⚠️ | Vista principal de gestión de alertas. |
| 3 | `chat_view.py` | `chat_view.py` | ✅ Mantener | ⚠️ | Vista en vivo del chat unificado. |
| 4 | `commands_view.py` | `commands_view.py` | ✅ Mantener | ✅ Aplicado | Plural coherente con `commands_controller` y colecciones. |
| 5 | `dashboard_view.py` | `dashboard_view.py` | ✅ Mantener | ⚠️ | Pantalla inicial de métricas y resumen. |
| 6 | `logs_view.py` | `logs_view.py` | ✅ Mantener | ✅ Aplicado | Plural coherente con `logs_controller` y `logs_storage`. |
| 7 | `music_view.py` | `music_view.py` | ✅ Mantener | ⚠️ | Vista del reproductor y peticiones de música. |
| 8 | `rewards_view.py` | `rewards_view.py` | ✅ Mantener | ⚠️ | Gestión de canjes y recompensas de canal. |
| 9 | `schedule_view.py` | `schedule_view.py` | ✅ Mantener | ⚠️ | Calendario y cronograma de directos. |
| 10 | `settings_view.py` | `settings_view.py` | ✅ Mantener | ⚠️ | Pantalla de configuración general de la aplicación. |
| 11 | `spam_view.py` | `spam_view.py` | ✅ Mantener | ⚠️ | Configuración de filtros antispam y moderación. |
| 12 | `timers_view.py` | `timers_view.py` | ✅ Mantener | ⚠️ | Vista de temporizadores programados de chat. |
| 13 | `widgets_view.py` | `widgets_view.py` | ✅ Mantener | ⚠️ | Galería y configuración de widgets para streaming. |

---

### 6. `frontend/widgets/` (15 archivos)

| # | Archivo Actual | Nombre Estandarizado | Acción | Estado | Justificación / Notas |
|---|---|---|---|---|---|
| 1 | `block_widget.py` | `block_widget.py` | ✅ Mantener | ✅ Aplicado | Primitiva UI; adopta sufijo `_widget.py`. |
| 2 | `category_search.py` | `category_search.py` | ✅ Mantener | ⚠️ | Selector con autocompletado para categorías de stream. |
| 3 | `clearable_line_edit.py` | `clearable_line_edit.py` | ✅ Mantener | ⚠️ | Campo de texto con botón de borrado rápido. |
| 4 | `color_picker.py` | `color_picker.py` | ✅ Mantener | ⚠️ | Selector visual de paleta RGB / HEX. |
| 5 | `controls_widget.py` | `controls_widget.py` | ✅ Mantener | ✅ Aplicado | Primitiva UI; adopta sufijo `_widget.py`. |
| 6 | `filter_header.py` | `filter_header.py` | ✅ Mantener | ⚠️ | Cabecera con filtros y buscador integrado. |
| 7 | `flow_layout.py` | `flow_layout.py` | ✅ Mantener | ⚠️ | Layout dinámico de ajuste fluido estilo Flexbox. |
| 8 | `no_wheel.py` | `no_wheel.py` | ✅ Mantener | ⚠️ | Modificador para evitar scroll involuntario en ComboBox / SpinBox. |
| 9 | `pagination_widget.py` | `pagination_widget.py` | ✅ Mantener | ✅ Aplicado | Primitiva UI de paginación de tablas. |
| 10 | `platform_controls.py` | `platform_controls.py` | ✅ Mantener | ⚠️ | Barra de botones de filtro por plataforma (Kick, Twitch, etc.). |
| 11 | `scalable_illustration.py` | `scalable_illustration.py` | ✅ Mantener | ⚠️ | Renderizador de arte vectorial SVG escalable. |
| 12 | `search_bar.py` | `search_bar.py` | ✅ Mantener | ⚠️ | Barra de búsqueda estándar con debounce. |
| 13 | `searchable_combo_box.py` | `searchable_combo_box.py` | ✅ Mantener | ⚠️ | Menú desplegable con filtro en tiempo real. |
| 14 | `segmented_control.py` | `segmented_control.py` | ✅ Mantener | ⚠️ | Conmutador de pestañas segmentadas tipo iOS/macOS. |
| 15 | `table_widget.py` | `table_widget.py` | ✅ Mantener | ✅ Aplicado | Primitiva UI de tabla estilizada. |

---

## 🗺️ Hoja de Ruta de Ejecución por Fases (Migration Roadmap)

### ✅ Fase 1: Interfaces & Contratos Base (Completado)
- Renombrado `backend/interfaces/` (10 archivos a `i_{dominio}.py`).
- Actualizado paquete `backend/interfaces/__init__.py` con retrocompatibilidad.
- **Comprobación:** Tests pasando (WT-1.5.9_06).

### ✅ Fase 2: Providers & Handlers (Completado)
- Renombrado providers de chat, music y voices (`{platform}_provider.py`, 10 archivos).
- Renombrado los 4 handlers (`spam_handler.py`, `logs_handler.py`, `music_handler.py`, `tts_handler.py`).
- Actualizado contenedor de DI y llamadas en workers.
- **Comprobación:** Tests pasando (WT-1.5.9_07).

### ✅ Fase 3: Servicios, Workers, DB, Models & Config (Completado)
- Renombrado servicios, workers, storages, models y config (`commands_service.py`, `timers_service.py`, `chat_pipeline.py`, `logs_service.py`, `widgets_service.py`, `auth_service.py`, `instance_service.py`, `twitch_rewards_worker.py`, `updater_worker.py`, `locale_defaults.py`, `alerts_storage.py`, `logs_storage.py`, `tokens_storage.py`, `alerts_models.py`, `alerts_queue.py`, `alerts_service.py`, `piper_manager.py`, `overlay_ws_client.py`).
- Actualizados todos los `__init__.py` y suites de prueba.
- **Comprobación:** Tests pasando (WT-1.5.9_08).

### ✅ Fase 4: Frontend — Dialogs, Views & Componentes (Completado)
- Renombrado 9 dialogs a `{dominio}_dialog.py` (`commands_dialog.py`, `message_dialog.py`, `piper_dialog.py`, `platform_dialog.py`, `tiktok_dialog.py`, `timers_dialog.py`, `updater_dialog.py`, `positioner_dialog.py`, `youtube_dialog.py`).
- Renombrado `command_view.py` -> `commands_view.py` y `log_view.py` -> `logs_view.py`.
- Renombrado `widget_card_component.py` -> `widget_card.py` y `log_controls.py` -> `logs_controls.py`.
- Actualizado `frontend/dialogs/__init__.py`, `frontend/views/__init__.py` y componentes.
- **Comprobación:** Tests pasando (WT-1.5.9_09).

### ✅ Fase 5: Frontend — Primitivas UI & Reubicación (Completado)
- Reubicado arquitectónicamente `base_view.py` desde `widgets/` a `frontend/views/base_view.py`.
- Renombrado primitivas en `widgets/` (`block_widget.py`, `controls_widget.py`, `pagination_widget.py`, `table_widget.py`).
- Implementado lazy export PEP 562 en `frontend.widgets` para compatibilidad absoluta sin ciclos circulares.
- Actualizados imports en vistas, componentes, dialogs y suites de prueba.
- **Comprobación Final:** 394 tests pasando (96 frontend + 298 backend). (WT-1.5.9_09).
