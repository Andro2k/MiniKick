...
*Comando de Volumen para Moderadores, Estandarización de Tags de Música, Optimización Arquitectónica de Controllers, Evolución de !playlist y Scroll Invisible en Chat Overlay*

En esta versión (v1.4.5), MiniKick continúa su evolución incorporando nuevas herramientas de moderación, refinando la arquitectura de rendimiento del backend y mejorando la interacción en pantalla para transmisiones en vivo. Introducimos el nuevo comando `!vol` para modificar el volumen de la música desde el chat por parte de moderadores, estandarizamos la nomenclatura de tags de plugin a `[PLUGIN_MUSIC_*]`, refactorizamos los controladores principales para eliminar cuellos de botella $O(N)$ y riesgos de ReDoS, evolucionamos el comando `!playlist` para consultas rápidas por posición y habilitamos scroll invisible e inteligente en la fuente de chat para OBS.

---

## 1. Comando de Volumen de Música (`!vol` / `!volume`)
...
> [!NOTE]
> Puedes ajustar o personalizar los permisos del comando `!vol` en la vista general de Comandos si deseas permitir que otros roles cambien el volumen.

---

## 2. Estandarización de Tags de Plugin de Música (`[PLUGIN_MUSIC_*]`)
...
## 3. Optimización de Rendimiento y Arquitectura en Controllers
...
* **Centralización de Verificaciones (DRY)**: Implementamos el helper `_require_active_provider` en `MusicController`, eliminando código duplicado de comprobación de proveedores.

---

## 4. Evolución del Comando `!playlist` (`!queue`, `!pl`)
...
## 5. Scroll Invisible e Inteligente en Overlay de Chat para OBS
...
> [!TIP]
> Puedes interactuar directamente con la ventana del chat overlay en tu software de transmisión usando la rueda del ratón sin que aparezca ninguna barra gris o antiestética en pantalla.
