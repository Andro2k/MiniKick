...
class GiphyService:
    ...
    _GIPHY_MEDIA_REGEX = re.compile(r"https?://media\d*\.giphy\.com/media/(?:v1\.[a-zA-Z0-9]+\/)?([a-zA-Z0-9]+)/", re.IGNORECASE)
    ...
    def resolve_gif(self, arg: str, api_key_override: str = "") -> Optional[str]:
        raw = (arg or "").strip()
        if not raw:
            return None
        ...
        if not active_key:
            logger.debug("[GiphyService] Cannot search query '%s': No GIPHY API key configured.", raw)
            return None
        ...
        return self._search_giphy(raw, active_key, cache_key)
