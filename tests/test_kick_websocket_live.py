# tests/test_kick_websocket_live.py
...
class KickWebsocketInspector:
    def __init__(self, channel_slug: str = None, room_id: int = None, cluster: str = PUSHER_CLUSTER, key: str = PUSHER_KEY, raw_mode: bool = False):
    ...
    def start(self):
        ...
        print(f"🚀 Conectando a Kick WebSocket (Pusher Cluster: {self.cluster})...")
        mode_str = "📦 MODO RAW (JSON 100% EN CRUDO)" if self.raw_mode else "💬 MODO RESUMIDO"
        print(f"💡 {mode_str} activo. Presione Ctrl+C para salir.\n" + "="*70)
    ...
    def _on_open(self, ws):
        print(f"🟢 Conexión establecida con éxito a Pusher ({datetime.now().strftime('%H:%M:%S')})")
    ...
    def _on_message(self, ws, raw: str):
        try:
            ...
            if event == "pusher:connection_established":
                print(f"📡 Suscribiendo a canales de chatroom #{self.room_id}...")
                ws.send(json.dumps({
            ...
            if self.raw_mode:
                ...
                try:
                    ...
                    print(json.dumps(parsed_outer, indent=2, ensure_ascii=False))
                except Exception:
                    print(raw)
                return
            ...
            if event == "App\\Events\\ChatMessageEvent":
                ...
                print(f"💬 [{now_str}] {badge_str} {user}: {msg}")
                ...
                print(f"🎁 [{now_str}] ¡REGALO DE SUBS! {gifter} regala {count} suscripciones! (a {gifted_to}...)")

            elif event == "App\\Events\\SubscriptionEvent":
                inner = json.loads(outer.get("data", "{}"))
                user = inner.get("username", "Desconocido")
                months = inner.get("months", 1)
                print(f"⭐ [{now_str}] ¡NUEVA SUSCRIPCIÓN! {user} se ha suscrito por {months} mes(es)!")

            elif event == "App\\Events\\StreamHostEvent":
                inner = json.loads(outer.get("data", "{}"))
                hoster = inner.get("hoster_username", "Alguien")
                viewers = inner.get("number_viewers", 0)
                print(f"🚀 [{now_str}] ¡RAID / HOST! {hoster} ha alojado el canal con {viewers} espectadores!")
                ...
                print(f"🚫 [{now_str}] BUSTED: {banned_user} fue baneado por {banned_by}")

            elif event == "App\\Events\\MessageDeletedEvent":
                inner = json.loads(outer.get("data", "{}"))
                msg_id = inner.get("id", {}).get("id") or inner.get("message", {}).get("id")
                print(f"🗑️ [{now_str}] Mensaje eliminado ID: {msg_id}")

            elif event == "App\\Events\\FollowersUpdate":
                inner = json.loads(outer.get("data", "{}"))
                print(f"❤️ [{now_str}] Nuevo seguidor / actualización de seguidores: {inner}")

            elif event == "App\\Events\\ViewersUpdate":
                inner = json.loads(outer.get("data", "{}"))
                viewers = inner.get("viewers")
                print(f"👁️ [{now_str}] Espectadores en vivo: {viewers}")

            else:
                print(f"⚡ [{now_str}] EVENTO RECIBIDO: '{event}'")
                try:
                    pretty_data = json.loads(outer.get("data", "{}"))
                    print(json.dumps(pretty_data, indent=2, ensure_ascii=False))
                except Exception:
                    print(outer)
    ...
    def _print_stats(self):
        ...
        print("\n" + "="*70)
        print("📊 RESUMEN DE LA PRUEBA WEBSOCKET DE KICK")
        print("="*70)
        print(f"⏱️ Tiempo transcurrido: {int(elapsed)} segundos")
        print("📈 Eventos Recibidos por Tipo:")
        ...
        print(f"💬 Promedio de chat: {msg_per_min} msgs/minuto")
        print("="*70)
...
def main():
    ...
    inspector = KickWebsocketInspector(channel_slug=args.channel, room_id=args.room_id, raw_mode=args.raw)
    try:
        inspector.start()
