...
# Eliminamos sys.path.append porque main.py ya maneja las rutas correctamente
...
class EditMediaModal(QDialog):
    ...
    # --- CORRECCIÓN CRÍTICA: Esta función ahora está dentro de la clase EditMediaModal ---
...
class PointsView(QWidget):
    ...
    def init_ui(self):
        ...
        obs_lay.addWidget(self.obs_link_input, 1) 
    ...
    def fetch_rewards(self):
        """Obtiene las recompensas del API de Kick."""
        if not self.sesion: return
        self.btn_fetch.setText("Cargando...")
        self.btn_fetch.setEnabled(False)
        
        # --- AQUÍ ESTÁ LA CORRECCIÓN: Quitamos self.sesion['access_token'] ---
        data = obtener_recompensas_kick() 
        
        self.combo_rewards.clear()
        if data:
            for rew in data: 
                self.combo_rewards.addItem(rew.get("title", "Sin Título"), rew.get("id", ""))
