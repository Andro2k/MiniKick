# frontend\common\__init__.py

from .paths import (
    resource_path,
    get_assets_path, resolve_icon_path,
)

from .icons import (
    ICON_SIZE_XS, ICON_SIZE_SM, ICON_SIZE_MD, ICON_SIZE_LG, ICON_SIZE_XL,
    get_icon, get_icon_colored, get_pixmap_colored, get_pixmap,
    create_circular_pixmap,
)

from .validators import (
    validate_trigger_prefix,
)

from .theme import (
    COLOR_NEUTRAL_950, COLOR_NEUTRAL_900, COLOR_NEUTRAL_850,
    COLOR_NEUTRAL_800, COLOR_NEUTRAL_750, COLOR_NEUTRAL_700,
    COLOR_NEUTRAL_500, COLOR_NEUTRAL_400, COLOR_NEUTRAL_200,
    COLOR_WHITE, COLOR_PURE_WHITE, COLOR_BLACK, COLOR_GREEN, COLOR_GREEN_DARK,
    COLOR_GREEN_HOVER, COLOR_GREEN_SOLID,
    COLOR_RED, COLOR_RED_DARK, COLOR_RED_HOVER, COLOR_RED_SOLID,
    COLOR_DANGER_SURFACE, COLOR_MEDIA_THUMB_BG,
    COLOR_AMBER, COLOR_AMBER_DARK,
    COLOR_BLUE, COLOR_BLUE_DARK, COLOR_PURPLE, COLOR_PURPLE_DARK,
    COLOR_KICK, COLOR_KICK_REWARDS, COLOR_KICK_BORDER,
    COLOR_TWITCH, COLOR_TWITCH_DARK, COLOR_TWITCH_LIGHT, COLOR_TWITCH_GLOW,
    COLOR_YOUTUBE, COLOR_YOUTUBE_DARK, COLOR_YOUTUBE_GLOW,
    COLOR_TIKTOK, COLOR_TIKTOK_DARK, COLOR_TIKTOK_GLOW,
    COLOR_BORDER_MUTED_FOCUS, COLOR_BORDER_TOP_SHINE,
    COLOR_BADGE_STREAMER, COLOR_BADGE_MODERATOR, COLOR_BADGE_VIP,
    COLOR_BADGE_OG, COLOR_BADGE_SUB, COLOR_BADGE_BG,
    COLOR_SWITCH_TRACK_OFF_0, COLOR_SWITCH_TRACK_OFF_1, COLOR_SWITCH_BORDER_OFF,
    COLOR_SWITCH_TRACK_ON_0, COLOR_SWITCH_TRACK_ON_1,
    COLOR_SWITCH_TRACK_DIS_0, COLOR_SWITCH_TRACK_DIS_1,
    COLOR_SWITCH_THUMB_OFF_0, COLOR_SWITCH_THUMB_OFF_1,
    COLOR_SWITCH_THUMB_ON_0, COLOR_SWITCH_THUMB_ON_1,
    COLOR_SWITCH_THUMB_DIS_0, COLOR_SWITCH_THUMB_DIS_1,
    COLOR_SYNTAX_VARIABLE,
    RADIUS_XS, RADIUS_SM, RADIUS_MD, RADIUS_LG, RADIUS_XL, RADIUS_PILL,
    SPACING_NONE, SPACING_2XS, SPACING_XS, SPACING_SM, SPACING_MD, SPACING_LG, SPACING_XL,
    MARGIN_NONE, MARGIN_2XS, MARGIN_XS, MARGIN_SM, MARGIN_MD,
    MARGIN_LG, MARGIN_XL,
    MARGIN_H_XS, MARGIN_H_SM, MARGIN_H_MD,
    MARGIN_V_XS, MARGIN_V_SM, MARGIN_V_MD,
    MARGIN_SETTING_ROW, MARGIN_SETTING_ROW_COMPACT, MARGIN_HERO,
    MARGIN_PLATFORMS_GRID, MARGIN_SECTION_HEADER, MARGIN_SECTION_HEADER_FIRST,
    MARGIN_TAB_PANEL, MARGIN_TAB_BAR,
    MARGIN_SCROLL_CONTENT, MARGIN_CHIP, MARGIN_TAG_BADGE,
    PADDING_INPUT, PADDING_BUTTON, PADDING_SPINBOX, PADDING_ITEM,
    PADDING_BADGE, PADDING_CHIP, PADDING_TAB, PADDING_MENU_ITEM,
    BORDER_DEFAULT, BORDER_SUBTLE, BORDER_MUTED, BORDER_TRANSPARENT,
    BORDER_FOCUS, BORDER_ERROR,
    PATH_ICON_HELP, PATH_ICON_CHEVRON_DOWN, PATH_ICON_CHEVRON_UP,
    PATH_ICON_CHEVRON_LEFT, PATH_ICON_CHEVRON_RIGHT, PATH_ICON_CHECK,
    PATH_ICON_CHECK_GREEN, PATH_ICON_CALENDAR,
    get_qss_colored_icon, get_global_qss, GLOBAL_QSS, get_swatch_qss,
    create_dark_palette,
)

from .markdown import markdown_to_github_html

__all__ = [
    "resource_path", "get_assets_path", "resolve_icon_path",
    "ICON_SIZE_XS", "ICON_SIZE_SM", "ICON_SIZE_MD", "ICON_SIZE_LG", "ICON_SIZE_XL",
    "get_icon", "get_icon_colored", "get_pixmap_colored", "get_pixmap",
    "create_circular_pixmap",
    "validate_trigger_prefix",
    "markdown_to_github_html",
    "COLOR_NEUTRAL_950", "COLOR_NEUTRAL_900", "COLOR_NEUTRAL_850",
    "COLOR_NEUTRAL_800", "COLOR_NEUTRAL_750", "COLOR_NEUTRAL_700",
    "COLOR_NEUTRAL_500", "COLOR_NEUTRAL_400", "COLOR_NEUTRAL_200",
    "COLOR_WHITE", "COLOR_BLACK", "COLOR_GREEN", "COLOR_GREEN_DARK",
    "COLOR_RED", "COLOR_RED_DARK", "COLOR_AMBER", "COLOR_AMBER_DARK",
    "COLOR_BLUE", "COLOR_BLUE_DARK", "COLOR_PURPLE", "COLOR_PURPLE_DARK",
    "COLOR_TWITCH", "COLOR_TWITCH_DARK", "COLOR_TWITCH_GLOW",
    "COLOR_YOUTUBE", "COLOR_YOUTUBE_DARK", "COLOR_YOUTUBE_GLOW",
    "COLOR_TIKTOK", "COLOR_TIKTOK_DARK", "COLOR_TIKTOK_GLOW",
    "RADIUS_XS", "RADIUS_SM", "RADIUS_MD", "RADIUS_LG", "RADIUS_XL", "RADIUS_PILL",
    "SPACING_NONE", "SPACING_2XS", "SPACING_XS", "SPACING_SM", "SPACING_MD", "SPACING_LG", "SPACING_XL",
    "MARGIN_NONE", "MARGIN_2XS", "MARGIN_XS", "MARGIN_SM", "MARGIN_MD",
    "MARGIN_LG", "MARGIN_XL",
    "MARGIN_H_XS", "MARGIN_H_SM", "MARGIN_H_MD", 
    "MARGIN_V_XS", "MARGIN_V_SM", "MARGIN_V_MD",
    "MARGIN_SETTING_ROW", "MARGIN_SETTING_ROW_COMPACT", "MARGIN_HERO",
    "MARGIN_PLATFORMS_GRID", "MARGIN_SECTION_HEADER", "MARGIN_SECTION_HEADER_FIRST",
    "MARGIN_TAB_PANEL", "MARGIN_TAB_BAR",
    "MARGIN_SCROLL_CONTENT", "MARGIN_CHIP", "MARGIN_TAG_BADGE",
    "PADDING_INPUT", "PADDING_BUTTON", "PADDING_SPINBOX", "PADDING_ITEM",
    "PADDING_BADGE", "PADDING_CHIP", "PADDING_TAB", "PADDING_MENU_ITEM",
    "BORDER_DEFAULT", "BORDER_SUBTLE", "BORDER_MUTED", "BORDER_TRANSPARENT",
    "BORDER_FOCUS", "BORDER_ERROR",
    "PATH_ICON_HELP", "PATH_ICON_CHEVRON_DOWN", "PATH_ICON_CHEVRON_UP",
    "PATH_ICON_CHEVRON_LEFT", "PATH_ICON_CHEVRON_RIGHT", "PATH_ICON_CHECK",
    "PATH_ICON_CHECK_GREEN", "PATH_ICON_CALENDAR",
    "get_qss_colored_icon", "get_global_qss", "GLOBAL_QSS", "get_swatch_qss",
    "create_dark_palette",
]
